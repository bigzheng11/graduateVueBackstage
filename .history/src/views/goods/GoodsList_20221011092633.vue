      ...
      <el-table
         :data="
            tableData
               ...
               .filter(
                  (tableData) =>
                     !search ||
                     tableData.goodsName.toLowerCase().includes(search.toLowerCase())
         ...
         <el-table-column label="是否已售卖" prop="isSell" width="100"> 
            <template slot-scope="scope">
               <p v-if="!scope.row.isSell" disable-transitions="false">{{scope.row.isSell?'已经卖了':'未售'}}</p>

               <el-tag type="warning" v-if="scope.row.isSell" disable-transitions="false">{{scope.row.isSell?'售馨':'未售'}}</el-tag>
            </template>
         </el-table-column>
